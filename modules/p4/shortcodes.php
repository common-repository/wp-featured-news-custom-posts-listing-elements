...
function wfnews_post_block_4_shortcode($atts){
...
******************/
		...
		if($heading_style == "style1") {
			...
			$list = '';
			$list .= '<div class="'.esc_attr($unique_css_class).' post-block post_block_4 post_block_4'.$current_time.' '.$blockborder.'" ';if($block_padding || $block_margin){ $list.='style="';if($block_padding){ $list.='padding:'.$block_padding.';'; } if($block_margin){ $list.='margin:'.$block_margin.';'; } $list.='"'; } $list.='>';
...
/*New $args end*/			
			...
			$list .= '<article class="style3 style-alt">
						...
						'; } $list.='	
							<div class="post-excerpt lg_meta">
			...
			$wp_query = $temp_query;
			wp_reset_query();
			//Second query section start from here	
					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							'; } $list.='
								...
								<a href="'.get_permalink().'" class="small-title rmore" ';if($button_font_data_inline_style || $button_font_size || $button_width){ $list.='style="'; if($button_font_data_inline_style){ $list.=$button_font_data_inline_style; } if($button_font_size){ $list.='font-size:'.esc_attr($button_font_size).' !important;'; }  $list.='"'; } $list.='>'.esc_html__('Read More','wfnews').'</a>
							</div>
						</div>
					</div>
				</article>'; 
...
add_shortcode('wfnews_post_block_4_post', 'wfnews_post_block_4_shortcode');
