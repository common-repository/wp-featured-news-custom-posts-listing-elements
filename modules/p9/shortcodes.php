...
function wfnews_post_block_9_shortcode($atts){
...
******************/	
		...
		if($heading_style == "style1") {
			...
			$list ='';
			$list .='<div class="post-block '.esc_attr($unique_css_class).' post_block_9 post_block_9'.$current_time.' '.$blockborder.'" ';if($block_padding || $block_margin){ $list.='style="';if($block_padding){ $list.='padding:'.$block_padding.';'; } if($block_margin){ $list.='margin:'.$block_margin.';'; } $list.='"'; } $list.='>';	
			...
			$list .= '<div class="row post_block_9_posts'.$current_time.'">';
...
add_shortcode('wfnews_post_block_9_post', 'wfnews_post_block_9_shortcode');
