...
function wfnews_post_block_8_shortcode($atts){
...
******************/
		...
		if($heading_style == "style1") {
			...
			$list ='';
			$list .='<div class="'.esc_attr($unique_css_class).' post-block post_block_8 post_block_8'.$current_time.' '.$blockborder.'" ';if($block_padding || $block_margin){ $list.='style="';if($block_padding){ $list.='padding:'.$block_padding.';'; } if($block_margin){ $list.='margin:'.$block_margin.';'; } $list.='"'; } $list.='>';
			if($heading){
			...
			$list .= '<div class="row style5 post_block_8_posts'.$current_time.'">';
...
add_shortcode('wfnews_post_block_8_post', 'wfnews_post_block_8_shortcode');
