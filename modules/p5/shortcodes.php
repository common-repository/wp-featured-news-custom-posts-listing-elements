...
function wfnews_post_block_5_shortcode($atts){
...
******************/
		...
		if($heading_style == "style1") {
			...
			if($blockborder=='yes'){ $blockborder='wfnews-make-border';
			}else{$blockborder='';}
			...
			if($sm_post_excerpt_limit==''){ $sm_post_excerpt_limit=15;}else{$sm_post_excerpt_limit=$sm_post_excerpt_limit;}
			...
			$list = '';
			$list .= '<div class="post-block '.esc_attr($unique_css_class).' post_block_5 post_block_5'.$current_time.' '.$blockborder.'" ';if($block_padding || $block_margin){ $list.='style="';if($block_padding){ $list.='padding:'.$block_padding.';'; } if($block_margin){ $list.='margin:'.$block_margin.';'; } $list.='"'; } $list.='>';
			if($heading){
...
/*New $args end*/			
			...
			$list .= '<div class="row"><div class="col-md-12">
			...
			if($showpost_item>0){		
			$list .= '<div class="row post_block_5_posts'.$current_time.'">';
			$arr1 = $post_array_new;
			$ar = array_merge($arr1);
...
/*New $args end*/			
			...
			$list .= '<div class="col-md-6">
						<article class="article-home">
							...
							'; } $list.='	
								<p ';if($excerpt_font_data_inline_style || $excerpt_font_size || $excerpt_line_height){ $list.='style="'; if($excerpt_font_data_inline_style){ $list.=$excerpt_font_data_inline_style; } if($excerpt_font_size){ $list.='font-size:'.esc_attr($excerpt_font_size).' !important;'; } if($excerpt_line_height){ $list.='line-height:'.esc_attr($excerpt_line_height).' !important;'; } $list.='"'; } $list.='>' . wfnews_excerpt($sm_post_excerpt_limit) . '</p>
							</div>
						</article>
					</div>'; 
				endwhile;endif;
...
add_shortcode('wfnews_post_block_5_post', 'wfnews_post_block_5_shortcode');
