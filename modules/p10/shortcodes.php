...
function wfnews_post_block_10_shortcode($atts){
extract( shortcode_atts( array(
		'type' => 'post',
        'category' => wfnews_default_category(),
		'current_time' => time(),
		'imageanimate' => 'no',
		'nextprevbtn' => 'no',
...
******************/
		...
		include('blockcategorybackground.php');	
			...
			if($itemcount==''){ $itemcount=2;}else{$itemcount=5;}
			...
			if($r_more_btn_color){
			include('r_more_btn_color.php');
			}
			if($load_more_btn_bg){
			include('load_more_btn_bg.php');
			}			
			$list ='';
			$list .='<div class="post-block '.esc_attr($unique_css_class).' post_block_10 post_block_10'.$current_time.' '.$blockborder.'" ';if($block_padding || $block_margin){ $list.='style="';if($block_padding){ $list.='padding:'.$block_padding.';'; } if($block_margin){ $list.='margin:'.$block_margin.';'; } $list.='"'; } $list.='>';
			if($heading){
...
/*New $args start*/
			...
			// post $args	
			$args = array('post_type' => 'post', 'posts_per_page' => $itemcount, 'cat' => $category, 'orderby' => $orderby, 'order' => $order);
			// post $args end
			
			} elseif($post_type == 'attachment') {
				
			// post $args	
			$args = array('post_type' => $post_type,'posts_per_page' => $itemcount,'orderby' => $orderby, 'order' => $order,  'post_mime_type' => 'image/jpeg,image/gif,image/jpg,image/png','post_status' => 'any' );			
			// post $args end	
			
			} else{

			// post $args	
			$args = array('post_type' => $post_type, 'posts_per_page' => $itemcount,  'orderby' => $orderby, 'order' => $order);				
			// post $args end	
...
/*New $args end*/			
			...
			$totalpostcount = $wp_query->found_posts;
						...
						<div class="row">
							...
							<div class="col-md-6">
								...
								'; } $list.='	
									...
									<a href="'.get_permalink().'" class="small-title rmore" ';if($button_font_data_inline_style || $button_font_size){ $list.='style="'; if($button_font_data_inline_style){ $list.=$button_font_data_inline_style; } if($button_font_size){ $list.='font-size:'.esc_attr($button_font_size).' !important;'; } $list.='"'; } $list.='>'.esc_html__('Read More','wfnews').'</a>
								</div>
							</div>
						</div>
					</article>'; 
...
add_shortcode('wfnews_post_block_10_post', 'wfnews_post_block_10_shortcode');
